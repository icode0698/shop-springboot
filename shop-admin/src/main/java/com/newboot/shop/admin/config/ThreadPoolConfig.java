...
public class ThreadPoolConfig {
     ...
     */
    @Value("${shop.admin.async.executor.core-pool-size}")
    private int corePoolSize;
    /**
     * 最大线程数，系统默认Integer.MAX_VALUE
     */
    @Value("${shop.admin.async.executor.max-pool-size}")
     ...
     * 就会延长响应时间，阻塞任务
     */
    @Value("${shop.admin.async.executor.queue-capacity}")
     ...
     */
    @Value("${shop.admin.async.executor.keep-alive-seconds}")
    private int keepAliveSeconds;
    /**
     * 线程池名前缀，用于监控识别
     */
    @Value("${shop.admin.async.executor.name-prefix}")
    private String namePrefix;
    /**
     * 允许核心线程超时（默认false）
     */
    @Value("${shop.admin.async.executor.core-thread-timeout}")
    private boolean allowCoreThreadTimeout;
    /**
     * 当任务完成后，长时间无待处理任务时，销毁线程池
     */
    @Value("${shop.admin.async.executor.task-shutdown}")
    private boolean waitForTasksToCompleteOnShutdown;
    /**
     * 线程池中任务的等待时间
     * 如果超过这个时间还没有销毁就强制销毁，以确保应用最后能够被关闭，而不是阻塞住
     */
    @Value("${shop.admin.async.executor.await-seconds}")
